...
f32 Read1_ADC_MCP3204(u8 channelNo)
{
   ...
   analogReading=adcVal*(5.0/4096);            /*12-bit Digital DATA,which is stored in adcVal varilable after
																							 ADC processing is multiplied by 
	                                             Step Size(smallest change in analog signal) = (Vref/2^12)         
                                               where as   Vref=5.0 volts, 2^12=4096*/   	 
